...
# Stage 1: Download AppImage and extract it; by doing a multi-stage build, we can
# discard the image cleanly but keep the download layer cached on the build system.

# Pin to amd64 because Dinamica only gets distributed for that platform; Tried running
# with multiarch support, but there are so many hard-coded system dependencies that this
# is really more trouble than it's worth.

# Use ubuntu:noble for the extractor because it is also the base image for
# rocker/r-ver:4.5.0
...
# cannot do a checksum on this, because new versions get uploaded to the same URL
ADD --chmod=0755 \
...
# Install bundled R package to system library
